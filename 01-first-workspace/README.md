...
3  Writing the descriptor
   workspace `hello` — the layer the descriptor names, and its identity
   ai-packs/_workspaces/hello/default/workspace.yaml
   ~/.deck/workspaces/hello/machine.yaml   (yours: packs_root, paths, targets)
   selected hello/default
   ~/.deck/workspaces/hello/toggles.yaml

4  Linking packs to repositories, by name
   linked   hello-api                          ~/lab/hello/ai-packs/_repos/hello-api
   linked   hello-cli                          ~/lab/hello/ai-packs/_repos/hello-cli
   linked   hello-schema                       ~/lab/hello/ai-packs/_repos/hello-schema
   shared   (every repository)                 ~/lab/hello/ai-packs/_workspaces/hello/default
   created 4 pack(s); each one is a skeleton to fill in
```

(deck prints absolute paths, not `~`. Every output on this page is real; the
only edit is that shortening.)

**The link between a pack and a repository is where the pack sits.** There is no
mapping table, because a mapping table is a file nobody keeps current. A pack in
`_repos/hello-api` applies to the repository of that name. A pack in
`_workspaces/hello/default` applies to every repository of the `hello`
workspace.

Two directory names carry the whole scheme:

| | What it names |
|---|---|
| `_repos/<repo>` | one repository, wherever it is checked out |
| `_workspaces/<name>/<scope>` | a workspace, and a phase of work inside it |

`<name>` can be `all`, meaning every workspace; `<scope>` can be `default`,
meaning the whole workspace rather than one initiative. So
`_workspaces/all/default` is what applies everywhere, and
`_workspaces/hello/security` would be one initiative in this workspace alone.
Exercise 3 uses that second axis; here everything sits in
`_workspaces/hello/default`.

> In a real team this would be `--packs-root ~/work/ai-packs`, pointing at a
> repository the team shares, so several products draw on one collection. That
> is what `_workspaces/<name>/` is for: one collection, one directory per
> product. The mechanism is identical.

Look at what appeared — and, more interestingly, at what did not:

```bash
ls -A                    # ai-packs/, and no .deck/
cat ai-packs/_workspaces/hello/default/workspace.yaml
ls ~/.deck/workspaces/hello/
cat ~/.deck/selected      # hello/default
```

**Your project gained nothing.** Not one file. The descriptor is in the
collection, where it can be reviewed; everything that is true of your machine
and nobody else's is in `~/.deck/workspaces/hello/`. A workspace deck manages
is a workspace whose git status is unchanged by deck.

The division that holds forever:

| | Where | Versioned |
|---|---|---|
| `~/.deck/workspaces/<name>/` | your machine | **no** — paths, hosts and tokens differ per person |
| `ai-packs/` | the team | **yes** — it is the knowledge, reviewed like code |

The last line of step 3, `selected hello/default`, is deck recording which
workspace you are in. One is selected at a time, and every later command answers
for it — which is why they work from anywhere, not only from inside the tree.
...
In `ai-packs/_workspaces/hello/default/workspace.yaml`, change two `impacts:` lines:
...
Replace `ai-packs/_workspaces/hello/default/config/gates.yaml` with:
...
In `ai-packs/_workspaces/hello/default/config/toggles.yaml`, under the `toggles:` key, indented
...
`deck packs` lists the four packs in merge order, most general first:
`hello/default` — the workspace layer — and then the per-repository ones. Order
is the whole point: a repository pack can override what the workspace layer
says, and never the other way round.
...
In `ai-packs/_workspaces/hello/default/workspace.yaml`, replace `backlog: []`:
...
`ai-packs/_repos/hello-schema/rules/contract.md`:
...
Delete the scaffolded `ai-packs/_repos/hello-schema/rules/example.md`. That is
the whole step — **there is nothing to declare.** `rules/`, `skills/` and
`agents/` are walked, not listed, so a file in the directory is a file that gets
placed.

That is a decision, not an omission. A directory that had to agree with a list
in `mount.yaml` was two statements of one fact, and one of them went stale the
first time somebody added a rule and forgot the entry.

And nothing says which repository this applies to, because nothing needs to: the
pack sits in `_repos/hello-schema`, so what it holds applies there and nowhere
else. A convention written for one repository does not become law across the
workspace by forgetfulness.
...
```
~/lab/hello/             your project. deck put nothing here.
├── ai-packs/            the team's. edges are in the descriptor; commands,
│   ├── _workspaces/     decisions and rules live here. Versioned.
│   │   └── hello/
│   │       └── default/
│   ├── _repos/
│   │   ├── hello-api/
...
├── selected                       hello/default
└── workspaces/hello/
    ├── machine.yaml               packs_root, paths, targets
    ├── toggles.yaml               the choices you recorded
    └── state/                     gate evidence, mount manifests
```

Two halves, and the split is not where a newcomer expects. It is not
project/team — it is *machine*/team. Nothing about your particular checkout
reaches the collection, and nothing the team decides depends on your paths.
