...
ok "pack new _common" -- deck pack new _common --dir ./org-packs/_common \
  --description "What every repository in the organisation follows"
cat > org-packs/_common/config/gates.yaml <<'YAML'
version: 1
...
ok "pack new catalog-schema" -- deck pack new catalog-schema \
  --dir ./ai-packs/catalog-schema --description "The contract everyone else reads"
cat > ai-packs/catalog-schema/config/gates.yaml <<'YAML'
version: 1
...
also "the organisation layer first" "1  _common                      every repository"
...
fails_saying "a second gate under one name is refused" \
  "already exists (from _common). Set" -- deck gate run --task ORG-3
...
exited "and an override reaches every repository, not only its own" 1
also "the pack's own repository passes" "ok   build        catalog-schema"
also "and a sibling runs a command written for someone else" "FAIL build        catalog-api"
ok "with the sibling looking for a file it does not have" \
  -- grep -q "No such file or directory: 'catalog.json'" \
     .deck/gates/logs/ORG-3/build-catalog-api.log
cat > ai-packs/catalog-schema/config/gates.yaml <<'YAML'
version: 1
...
ok "pack new, bound to the initiative" -- deck pack new catalogue-work \
  --dir ./org-packs/catalogue-work --scope catalogue --from-workspace \
  --description "What this initiative needs, and only while it runs"
ok "detect.yaml binds it to the scope" \
  -- grep -q "^scope: catalogue" org-packs/catalogue-work/config/detect.yaml
...
cat > org-packs/catalogue-work/config/gates.yaml <<'YAML'
version: 1
...
YAML
cat > org-packs/catalogue-work/rules/history.md <<'MD'
---
paths: ["*.json"]
---
...
MD
rm org-packs/catalogue-work/rules/example.md
python3 - <<'PY'
import pathlib
p = pathlib.Path("org-packs/catalogue-work/config/mount.yaml")
...
also "and it merges between the organisation and the repository" \
  "2  catalogue-work               scope catalogue"
...
also "the rule reached both scope repositories" "2 artifact(s)"
ok "it is a file, not a symlink" -- test -f catalog-schema/.claude/rules/deck-history.md
ok "and Claude Code would read it, which a symlink there would not be" \
  -- test ! -L catalog-schema/.claude/rules/deck-history.md
...
capture -- deck ask fold "$ASK" --as toggle --into org-packs/catalogue-work \
...
exited "ask fold" 0
also "into the pack the initiative owns" "folded $ASK into catalogue-work as a toggle"
ok "and the entry lands unasked, which is a second decision" \
  -- grep -q "^    askable: false" org-packs/catalogue-work/config/toggles.yaml
