...
# The project gains nothing: the descriptor is in the collection and the
# machine state is in the home directory. A check that the project stayed
# untouched is the one a reader would most want, so it is asserted, not implied.
ok "the descriptor landed in the collection" -- test -f ai-packs/_workspaces/hello/default/workspace.yaml
ok "and the project gained no .deck/" -- test ! -e .deck
ok "machine state went to the home directory" -- test -f "$DECK_HOME_STATE/workspaces/hello/machine.yaml"
ok "the workspace is selected" -- grep -qx "hello/default" "$DECK_HOME_STATE/selected"
ok "a pack per repository, under _repos/" -- test -d ai-packs/_repos/hello-api
ok "and a workspace layer, under _workspaces/" -- test -d ai-packs/_workspaces/hello/default

# ------------------------------------------------------------------- 4
python3 - <<'PY'
import pathlib
p = pathlib.Path("ai-packs/_workspaces/hello/default/workspace.yaml"); s = p.read_text()
...
# ------------------------------------------------------------------- 5
cat > ai-packs/_workspaces/hello/default/config/gates.yaml <<'YAML'
gates:
  - id: readme
...
ok "evidence outlives the session" -- test -f "$DECK_HOME_STATE/workspaces/hello/gates/hello-1.json"
...
# ------------------------------------------------------------------- 6
cat >> ai-packs/_workspaces/hello/default/config/toggles.yaml <<'YAML'
  - id: schema_compat
    group: quality
...
# ------------------------------------------------------------------- 9
cat > ai-packs/_repos/hello-schema/rules/contract.md <<'MD'
---
paths: ["*.yaml", "*.json"]
---
...
MD
rm ai-packs/_repos/hello-schema/rules/example.md
# Nothing is declared. `rules/` is walked, so writing the file is the whole
# step, and a `rules:` key in mount.yaml is read by nothing. The exercise
# used to write one, and the mount check passed on the walk regardless — a
# check passing for a reason its own setup contradicted.
