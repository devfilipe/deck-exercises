...
`setup.sh` makes the four directories into repositories, runs `deck setup` to
create the machine half under `~/.deck/workspaces/polyglot/`, and copies in the
descriptor and the board this exercise ships. Your project directory gains only
`docs/board.yaml` — deck keeps nothing else here. Expect **5 gates in 9 runs, all green**, and `6 warning(s)`
...
Three artifacts, and notice where each one landed:

```bash
ls .claude/rules                       # the workspace layer, once
ls hello-commands/.claude/rules
ls hello-core/.claude/rules
```

`deck-layering.md` — the rule from `_workspaces/polyglot/default` — lands **once,
at the workspace root**. It is a statement about the workspace, so four copies
would say the same thing four times and be read four times.
`deck-adding-a-language.md` lands only in `hello-commands` and
`deck-resolution.md` only in `hello-core`, because those packs sit in
`_repos/<that repository>`. Each pack stayed in its own scope without anyone
declaring it: **where the pack sits is the declaration.**
...
runtime ignores. Edit the pack, not the copy; the manifest under
`~/.deck/workspaces/polyglot/mounts/` lists exactly what was placed, and
`unmount` removes exactly that.
