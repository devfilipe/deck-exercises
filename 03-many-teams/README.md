...
```bash
deck pack new org-baseline --dir ./org-packs/_workspaces/all/default \
  --description "What every repository in the organisation follows"
```

**A pack's directory says what it applies to, and nothing else does.** A
collection has two top-level directories and they answer two different
questions:
...
`<name>` may be `all`, meaning every workspace this collection serves; `<scope>`
may be `default`, meaning the whole of it rather than one initiative. So
`_workspaces/all/default` is the organisation layer: it applies everywhere,
without naming anybody. Later in this exercise you will write
`_workspaces/all/catalogue`, which is the same reach narrowed to one initiative.

Layers merge most general first — `all/default`, then `all/<scope>`, then
`<name>/default`, then `<name>/<scope>`, then `_repos/<repository>` — so a
repository can override the organisation and never the other way round.

Replace `org-packs/_workspaces/all/default/config/gates.yaml` with the ladder the organisation
runs everywhere:
...
```bash
deck pack new catalog-schema --dir ./ai-packs/_repos/catalog-schema \
  --description "The contract everyone else reads"
```

Replace `ai-packs/_repos/catalog-schema/config/gates.yaml`:
...
packs   2 in play, merged most general first
  ...
  1  all/default                  every repository     0 toggle(s) · 3 gate(s) · 0 rule(s)
     …/03-many-teams/org-packs/_workspaces/all/default
  2  catalog-schema               catalog-schema       0 toggle(s) · 1 gate(s) · 0 rule(s)
     …/03-many-teams/ai-packs/_repos/catalog-schema
...
## What `overrides:` does, and how far it reaches

`catalog-schema` might reasonably want its own `build`. Try it, in
`ai-packs/_repos/catalog-schema/config/gates.yaml`:
...
```
deck: catalog-schema: gate `build` already exists (from all/default). Set `overrides: true` to extend it deliberately.
```
...
```
  ...
  ok   build        catalog-api                0.1s
  ok   build        storefront-web             0.0s
  ok   build        deploy-scripts             0.0s
```

Four greens, and they are not the same green. The summary line cannot tell you
that; the logs can:

```
$ grep -c "the contract parses" .deck/gates/logs/ORG-3/build-catalog-schema.log
1
$ grep -c "the contract parses" .deck/gates/logs/ORG-3/build-catalog-api.log
0
```

`catalog-api` ran `${repo.build}` — its own `python3 -m json.tool routes.json`,
straight from the descriptor — and never saw the override at all.

**An override reaches exactly as far as the pack that wrote it.** This one sits
in `_repos/catalog-schema`, so it is scoped to `catalog-schema` the same way
`required-fields` is — the shared `build` gate keeps running, unchanged, in the
other three. The gate id stays single and workspace-wide, which is what lets
`deck gate report` compare a run against the last one; what changed is one
repository's command.

That containment is a property of **where the pack sits**, not of the flag. An
override written in a pack that speaks for nobody in particular — a workspace
layer, `_workspaces/all/default` — has nothing to be scoped to, and it does
reach every repository the original did. That is the one to read twice before
merging.

> If a repository needs a check no other one has, a **new id** in its own pack
> is still the clearer answer — which is what `required-fields` is. `overrides:`
> is for when the shared gate is the right gate and one repository needs it to
> run differently.

Delete that entry before moving on. The organisation's `build` gate never
hard-coded a command in the first place: `${repo.build}` asks the descriptor,
and the answer is per repository by construction.
...
```bash
deck pack new catalogue-work --dir ./org-packs/_workspaces/all/catalogue \
...
`org-packs/_workspaces/all/catalogue/config/gates.yaml`:
...
`org-packs/_workspaces/all/catalogue/rules/history.md` (delete the scaffolded
`rules/example.md`):
...
Nothing to declare: the file being in `rules/` is what places it. Delete the
scaffolded `rules/example.md` so only yours is left.

```bash
deck packs                      # all/catalogue: not in play
deck --scope catalogue packs    # all/catalogue: 2nd, between all/default and catalog-schema
```

```
not in play   bound to an initiative, and loaded only while it is active
  all/catalogue                scope catalogue      0 toggle(s) · 1 gate(s) · 1 rule(s)
     …/03-many-teams/org-packs/_workspaces/all/catalogue
...
```
   named explicitly     `packs:` in the descriptor
          ↓
   all/default          every repository, by where the pack sits
          ↓
   all/catalogue        every repository the initiative holds, while it is active
          ↓
...
mounted for task CAT-1
  ...
  rule     catalog-schema               …/03-many-teams/org-packs/_workspaces/all/catalogue/rules/history.md
  rule     catalog-api                  …/03-many-teams/org-packs/_workspaces/all/catalogue/rules/history.md
...
```bash
deck ask fold "$ASK" --as toggle --into org-packs/_workspaces/all/catalogue \
...
```yaml
# org-packs/_workspaces/all/catalogue/config/toggles.yaml
...
| **`overrides:`** | an override says what it changes and nothing else; it reaches as far as the pack that wrote it, which for a repository pack is that one repository, and for a workspace layer is everybody |
