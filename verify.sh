...
# deck keeps machine state — the selected workspace, the machine overlay, the
# gate evidence — under `~/.deck`, so a verification run would write into the
# reader's own. It did: a run left a workspace behind in mine and overwrote the
# selection, and the next deck command in an unrelated project answered for the
# exercise. Pointing DECK_HOME_STATE at the throwaway directory makes the run
# leave a machine exactly as it found it, which is what the header promises.
export DECK_HOME_STATE="$WORK/.home-state"
mkdir -p "$DECK_HOME_STATE"
